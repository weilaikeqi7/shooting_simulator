...
// Keep scan chatter off by default; continuous printf during BLE scan can starve RGB refresh.
#define GAP_DBG_PRINTF(...) do { } while (0)

#define LOOKBON_REMOTE_UUID 0xaf30
static const uint8_t s_lookbon_remote_bda[6] = {0x68, 0xC4, 0x92, 0x10, 0xA3, 0x66};

static bool lookbon_remote_addr_matches(const uint8_t bda[6])
{
    return memcmp(bda, s_lookbon_remote_bda, sizeof(s_lookbon_remote_bda)) == 0;
}

static bool lookbon_remote_name_matches(const uint8_t *name, uint8_t name_len)
{
    static const char expected_name[] = "LOOKBON";
    const size_t expected_len = sizeof(expected_name) - 1;

    return name && name_len == expected_len && memcmp(name, expected_name, expected_len) == 0;
}
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    const bool is_hid_service = uuid == ESP_GATT_UUID_HID_SVC;
    const bool is_lookbon_remote = lookbon_remote_addr_matches(scan_rst->bda) ||
                                   (uuid == LOOKBON_REMOTE_UUID && lookbon_remote_name_matches(adv_name, adv_name_len));

    if ((is_hid_service || is_lookbon_remote) && !find_scan_result(scan_rst->bda, ble_scan_results)) {
        if (is_lookbon_remote && !is_hid_service) {
            ESP_LOGI(TAG, "Adding LOOKBON remote as BLE HID scan candidate");
        }
...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc)
{
    ...
    const bool is_lookbon_addr = lookbon_remote_addr_matches(disc->addr.val);

    for (int i = 0; i < fields.num_uuids16; i++) {
        const uint16_t uuid = ble_uuid_u16(&fields.uuids16[i].u);
        const bool is_hid_service = uuid == BLE_HID_SVC_UUID;
        const bool is_lookbon_remote = is_lookbon_addr ||
                                       (uuid == LOOKBON_REMOTE_UUID && lookbon_remote_name_matches(adv_name, adv_name_len));

        if ((is_hid_service || is_lookbon_remote) && !find_scan_result(disc->addr.val, ble_scan_results)) {
            if (is_lookbon_remote && !is_hid_service) {
                ESP_LOGI(TAG, "Adding LOOKBON remote as BLE HID scan candidate");
            }
